...
import com.example.onlinestudent.profilegeneral.course.AttendanceModelItem
import com.example.onlinestudent.profilegeneral.course.ModelnameCourseItem
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.async

class ViewModelProfile:ViewModel() { val  coursnam= MutableLiveData<String>("")
val  coursid= MutableLiveData<String>("")
val  coursidapp= MutableLiveData<String>("")
...
var atten = MutableLiveData<List<ModelnameCourseItem>>()
//var noti = MutableLiveData<List<notificationModelItem>>()
///////////////////list artical such as model in Adapter
///////call api on response
fun getdatafromserver(context: Context, coid:String, apid:String){
    val pref= context.getSharedPreferences("PREFS_NAME", Context.MODE_PRIVATE)

    val valueid=pref.getString("ID", null)

    CoroutineScope(Dispatchers.IO).async {
        val response            = ConnectionAttendance.Retrofit_connection().MyAttend(coid,valueid!!)
...
//
//            }}
//    ConnectionNotification.Retrofit_connection().notification("11").enqueue(object :
...
fun selectattcourse(id:String, context: Context, stud:String){


    CoroutineScope(Dispatchers.IO).async {
        val response            =ConnectionAttendance.Retrofit_connection().diplomaAtendsom(stud,id)
        CoroutineScope(Dispatchers.Main).async {
            if(response.isSuccessful){
                attend.value=response.body()!!
            }else{
...
fun contentcorse(context: Context){
    val pref= context.getSharedPreferences("PREFS_NAME", Context.MODE_PRIVATE)
